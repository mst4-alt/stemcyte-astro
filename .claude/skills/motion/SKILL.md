...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `secondsky-claude-skills-motion`                                |
| Version    | 1.0.4                                     |
| Author     | secondsky                                 |
...
**GitHub:** [secondsky/claude-skills](https://github.com/secondsky/claude-skills) — ⭐ 71 | Forks: 5
...
motion is a React animation library (Framer Motion) providing a declarative API and ready-to-use motion components for building interactive UI animations. Use it for drag-and-drop interactions, gesture-driven effects, scroll-based reveals, complex transition choreography, SVG path morphing, and spring-based physics animations. Core features include variants, AnimatePresence for mounting/unmounting transitions, layout animations, animation controls (useAnimation), motion values (useMotionValue/useTransform), drag controls and constraints, and scroll/in-view hooks. The Skill is valuable when you need smooth microinteractions, route or component transitions, or coordinated timelines without manual DOM manipulation. Tips include tree-shaking imports to reduce bundle size, tuning spring parameters to avoid physics errors, using reduced-motion preferences for accessibility, and monitoring performance on low-end devices. Best for React apps needing polished, maintainable animations.
...
```bash
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion
```
...
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install secondsky-claude-skills-motion --version 1.0.4
```
...
```bash
npx -y @lobehub/market-cli skills rate secondsky-claude-skills-motion --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment secondsky-claude-skills-motion -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment secondsky-claude-skills-motion
```
...
```bash
npx -y @lobehub/market-cli skills comments secondsky-claude-skills-motion
```
