    ...
    <>
      <div className="pf-page" id="pricing-form">
        ...
        {step === 2 && (<form className="pf-checkout" autoComplete="on" onSubmit={e => e.preventDefault()}>
          ...
          <div className="pf-form-row"><div><input className="pf-form-input" name="given-name" autoComplete="given-name" placeholder="First name" value={form.firstName} onChange={uf('firstName')} /></div><div><input className="pf-form-input" name="family-name" autoComplete="family-name" placeholder="Last name" value={form.lastName} onChange={uf('lastName')} /></div></div>
          ...
          {primaryIsBM && <div className="pf-form-row"><div><input className="pf-form-input" type="date" placeholder="Due date" value={form.dueDate} onChange={uf('dueDate')} /></div><div><input className="pf-form-input" type="date" name="bday" autoComplete="bday" placeholder="Your birthday" value={form.birthday} onChange={uf('birthday')} /></div></div>}
          <div className="pf-form-row"><div><input className="pf-form-input" type="email" name="email" autoComplete="email" placeholder="Email address" value={form.email} onChange={uf('email')} /></div><div><input className="pf-form-input" type="tel" name="tel" autoComplete="tel" placeholder="Phone number" value={form.phone} onChange={uf('phone')} /></div></div>
          <div className="pf-form-full"><input className="pf-form-input" name="street-address" autoComplete="street-address" placeholder="Street address (kit delivery)" value={form.street} onChange={uf('street')} /></div>
          <div className="pf-form-row-3"><div><input className="pf-form-input" name="address-level2" autoComplete="address-level2" placeholder="City" value={form.city} onChange={uf('city')} /></div><div><input className="pf-form-input" name="address-level1" autoComplete="address-level1" placeholder="State" value={form.state} onChange={uf('state')} /></div><div><input className="pf-form-input" name="postal-code" autoComplete="postal-code" placeholder="ZIP code" value={form.zip} onChange={uf('zip')} /></div></div>
          {parents.map((p) => (<div key={p.id} className="pf-parent-block"><div className="pf-parent-block-header"><span className="pf-parent-block-title">Additional parent/guardian</span><button className="pf-remove-parent" onClick={() => removeParent(p.id)}>Remove</button></div><div className="pf-form-row"><div><input className="pf-form-input" placeholder="First name" value={p.firstName} onChange={e => setParentField(p.id, 'firstName', e.target.value)} /></div><div><input className="pf-form-input" placeholder="Last name" value={p.lastName} onChange={e => setParentField(p.id, 'lastName', e.target.value)} /></div></div><div className="pf-form-full"><select className="pf-form-select" value={p.role} onChange={e => setParentField(p.id, 'role', e.target.value)}><option value="">Relationship to baby</option><option value="birth_mother" disabled={primaryRole === 'birth_mother' || parents.some(o => o.id !== p.id && o.role === 'birth_mother')}>Birth mother{(primaryRole === 'birth_mother' || parents.some(o => o.id !== p.id && o.role === 'birth_mother')) ? ' (already selected)' : ''}</option><option value="mother">Mother</option><option value="father">Father</option><option value="surrogate">Surrogate</option><option value="other">Other guardian</option></select></div>{p.role === 'birth_mother' && <div className="pf-form-row"><div><input className="pf-form-input" type="date" placeholder="Due date" value={p.dueDate} onChange={e => setParentField(p.id, 'dueDate', e.target.value)} /></div><div><input className="pf-form-input" type="date" placeholder="Birthday" value={p.birthday} onChange={e => setParentField(p.id, 'birthday', e.target.value)} /></div></div>}<div className="pf-form-full"><input className="pf-form-input" type="tel" placeholder="Phone number" value={p.phone} onChange={e => setParentField(p.id, 'phone', e.target.value)} /></div></div>))}
