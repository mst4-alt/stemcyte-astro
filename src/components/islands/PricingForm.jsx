    ...
    <>
      <div className="pf-page" id="pricing-form">
        ...
        {step === 2 && (<form className="pf-checkout" autoComplete="on" onSubmit={e => e.preventDefault()} name="enrollment">
          <div style={{display:'flex',gap:6,marginBottom:16}}><button type="button" style={{padding:'5px 12px',borderRadius:100,border:formLayout==='new'?'2px solid #6C1A55':'1px solid #E8E2DC',background:formLayout==='new'?'rgba(108,26,85,0.05)':'#fff',color:formLayout==='new'?'#6C1A55':'#8A857A',fontFamily:'Lato,sans-serif',fontSize:10,fontWeight:700,cursor:'pointer'}} onClick={()=>setFormLayout('new')}>Current: role after address</button><button type="button" style={{padding:'5px 12px',borderRadius:100,border:formLayout==='old'?'2px solid #6C1A55':'1px solid #E8E2DC',background:formLayout==='old'?'rgba(108,26,85,0.05)':'#fff',color:formLayout==='old'?'#6C1A55':'#8A857A',fontFamily:'Lato,sans-serif',fontSize:10,fontWeight:700,cursor:'pointer'}} onClick={()=>setFormLayout('old')}>Original: role after name</button></div>
          ...
          {formLayout === 'new' && <><div className="pf-divider" /><div className="pf-form-full"><select className="pf-form-select" value={primaryRole} onChange={e => setPrimaryRole(e.target.value)} autoComplete="off"><option value="">Relationship to baby</option><option value="birth_mother" disabled={parents.some(p => p.role === 'birth_mother')}>Birth mother{parents.some(p => p.role === 'birth_mother') ? ' (already selected)' : ''}</option><option value="mother">Mother</option><option value="father">Father</option><option value="surrogate">Surrogate</option><option value="other">Other guardian</option></select></div>
          {primaryIsBM && <div className="pf-form-row"><div><input className="pf-form-input" type="date" placeholder="Due date" value={form.dueDate} onChange={uf('dueDate')} autoComplete="off" /></div><div><input className="pf-form-input" type="date" name="bday" autoComplete="bday" placeholder="Your birthday" value={form.birthday} onChange={uf('birthday')} /></div></div>}</>}
