...
export default function PricingForm() {
  ...
  const goStep = (n) => { if (n > 1 && !plan) { setPlanNudge(true); document.querySelector('.pf-list')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => setPlanNudge(false), 2000); return; } setStep(n); setTimeout(scrollToForm, 50); };
  const validateStep2 = () => { const allRoles = [primaryRole, ...parents.map(p => p.role)]; if (!allRoles.includes('birth_mother')) { setErrorMsg('A birth mother must be listed \u2014 either as the primary contact or as an additional parent.'); return; } setErrorMsg(''); goStep(3); };
  ...
  return (
    <>
      <div className="pf-page" id="pricing-form">
        ...
        {step === 2 && (<form className="pf-checkout" autoComplete="on" onSubmit={e => e.preventDefault()}>
          ...
          <div className="pf-form-row"><div><input className="pf-form-input" name="given-name" autoComplete="given-name" placeholder="First name" /></div><div><input className="pf-form-input" name="family-name" autoComplete="family-name" placeholder="Last name" /></div></div>
          <div className="pf-form-full"><select className="pf-form-select" value={primaryRole} onChange={e => setPrimaryRole(e.target.value)}><option value="">Relationship to baby</option><option value="birth_mother" disabled={parents.some(p => p.role === 'birth_mother')}>Birth mother{parents.some(p => p.role === 'birth_mother') ? ' (already selected)' : ''}</option><option value="mother">Mother</option><option value="father">Father</option><option value="surrogate">Surrogate</option><option value="other">Other guardian</option></select></div>
          {primaryIsBM && <div className="pf-form-row"><div><input className="pf-form-input" type="date" placeholder="Due date" /></div><div><input className="pf-form-input" type="date" name="bday" autoComplete="bday" placeholder="Your birthday" /></div></div>}
          <div className="pf-form-row"><div><input className="pf-form-input" type="email" name="email" autoComplete="email" placeholder="Email address" /></div><div><input className="pf-form-input" type="tel" name="tel" autoComplete="tel" placeholder="Phone number" /></div></div>
          <div className="pf-form-full"><input className="pf-form-input" name="street-address" autoComplete="street-address" placeholder="Street address (kit delivery)" /></div>
          <div className="pf-form-row-3"><div><input className="pf-form-input" name="address-level2" autoComplete="address-level2" placeholder="City" /></div><div><input className="pf-form-input" name="address-level1" autoComplete="address-level1" placeholder="State" /></div><div><input className="pf-form-input" name="postal-code" autoComplete="postal-code" placeholder="ZIP code" /></div></div>
          {parents.map((p) => (<div key={p.id} className="pf-parent-block"><div className="pf-parent-block-header"><span className="pf-parent-block-title">Additional parent/guardian</span><button className="pf-remove-parent" onClick={() => removeParent(p.id)}>Remove</button></div><div className="pf-form-row"><div><input className="pf-form-input" placeholder="First name" /></div><div><input className="pf-form-input" placeholder="Last name" /></div></div><div className="pf-form-full"><select className="pf-form-select" value={p.role} onChange={e => setParentRole(p.id, e.target.value)}><option value="">Relationship to baby</option><option value="birth_mother" disabled={primaryRole === 'birth_mother' || parents.some(o => o.id !== p.id && o.role === 'birth_mother')}>Birth mother{(primaryRole === 'birth_mother' || parents.some(o => o.id !== p.id && o.role === 'birth_mother')) ? ' (already selected)' : ''}</option><option value="mother">Mother</option><option value="father">Father</option><option value="surrogate">Surrogate</option><option value="other">Other guardian</option></select></div>{p.role === 'birth_mother' && <div className="pf-form-row"><div><input className="pf-form-input" type="date" placeholder="Due date" /></div><div><input className="pf-form-input" type="date" placeholder="Birthday" /></div></div>}<div className="pf-form-full"><input className="pf-form-input" type="tel" placeholder="Phone number" /></div></div>))}
          ...
          <div className="pf-form-full"><input className="pf-form-input" placeholder="Hospital or birth center" /></div>
          <div className="pf-form-full"><input className="pf-form-input" placeholder="Doctor's name" /></div>
