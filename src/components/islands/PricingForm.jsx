...
export default function PricingForm() {
  ...
  const goStep = (n) => { if (n > 1 && !plan) { setPlanNudge(true); document.querySelector('.pf-list')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => setPlanNudge(false), 2000); return; } if (n === 1 && !cellyOpen) { setBannerVisible(true); cellyBannerInjected.current = false; cellyAvInjected.current = false; } setStep(n); setTimeout(scrollToForm, 50); };
